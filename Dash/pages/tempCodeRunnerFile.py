...
# Define the layout for prediction
    ...
    children=[
        ...
        html.Div(
            children=[
                html.Div(
                    children=dcc.Graph(
                        id="daily-stats-predict", config={"displayModeBar": False},
                    ),
                    className="card",
                ),
            ],
...
@app.callback(
    Output("daily-stats-predict", "figure"),
    [
        Input("parameter-filter-predict", "value"),
        Input("date-range-predict", "start_date"),
        Input("date-range-predict", "end_date"),
...
def update_daily_stats_predict(selected_parameter, start_date, end_date):
    mask = (
        (prediction["DATETIMEDATA"] >= start_date)
        & (prediction["DATETIMEDATA"] <= end_date)
    )
